...
export const onRequestGet: PagesFunction<Env> = async (context) => {
    ...
    const category = await getCached<Category>(context, `/obj/categories/${categoryID}`, async () => {
        return await (new Category(context.env.DB)).get(categoryID);
    });
    const posts = await getCached<Array<Post>>(context, "/obj/posts/categories", async () => {
